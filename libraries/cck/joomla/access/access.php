...
class CCKAccess
{
	...
	public static function getAssetRules($asset, $recursive = false, $component = '', $section = '')
	{
		...
		$rules	=	new CCKRules( '', $component, $section );
	...
	public static function getGroupsByUser($userId, $recursive = true)
	{
		...
		if (!isset($results[$storeId]))
		{
			...
			else
			{
				...
				$query->join('LEFT', '#__usergroups AS a ON a.id = map.group_id');
	...
	public static function getAuthorisedViewLevels($userId)
	{
		...
		if (empty(self::$viewLevels))
		{
			...
			$query->from($query->quoteName('#__viewlevels'));
	 ...
	 */
	public static function getActions($component, $section = 'component', $index = '')
	{
		$actions = array();
		...
		if (defined('JPATH_ADMINISTRATOR') && is_file(JPATH_ADMINISTRATOR.'/components/'.$component.'/access.xml')) {
			...
			foreach ($xml->children() as $child)
			{
				if ($section == (string) $child['name']) {
					...
					{
						if ( $index != '' ) {
							$idx			=	(string)$action[$index];
							$actions[$idx]	=	(object) array(
													'name' => (string)$action['name'],
													'title' => (string)$action['title'],
													'description' => (string)$action['description'],
													'input_type' => (string)@$action['input_type'],
													'var_type' => (string)@$action['var_type']
												);
						} else {
