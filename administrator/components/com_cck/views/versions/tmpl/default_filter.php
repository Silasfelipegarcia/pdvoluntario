...
$clear	=	"document.getElementById('filter_e_type').value='1';";
if ( $this->js['filter'] ) {
...
<div class="<?php echo $this->css['filter']; ?>" id="filter-bar">
	<div class="<?php echo $this->css['filter_search']; ?>">
        <?php
		echo JCckDev::getForm( $cck['core_version_location_filter'], $this->state->get( 'filter.location' ), $config );
	...
	<div class="<?php echo $this->css['filter_select']; ?>">
		...
		echo JCckDev::getForm( $cck['core_version_e_type_filter'], $this->state->get( 'filter.e_type' ), $config, array( 'css'=>'small span12' ) );
